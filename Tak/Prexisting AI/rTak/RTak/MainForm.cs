...
namespace TakGame_WinForms
{
    public partial class MainForm : Form
    {
        ...
        private void SetResultCode(int eval)
        {
            ...
            {
                string condition = "F";
                if (Math.Abs(eval) > Evaluation.FlatWinEval)
                    condition = "R";
                if (eval > 0)
        ...
        private void listAiLevel_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(s.Substring(s.Length - 1), out level))
            {
                ...
                string desc = string.Format("RTak AI level {0}", level, _ai.Evaluator.Name);
                if ((_game.Ply & 1) == 0)
                    _gameRecord.Player1 = desc;
                else
                    _gameRecord.Player2 = desc;
        ...
        const string ShareLinkPrefix = "https://jsfiddle.net/bwochinski/043hpzwu/embedded/result/";

        private void miCopyLink_Click(object sender, EventArgs e)
        {
            var ptn = SaveToString();
            var url = ShareLinkPrefix + "?ptn=" + System.Uri.EscapeUriString(ptn);
        ...
        private void miPaste_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(this, "No text was found on the clipboard", "RTak paste error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            if (s.StartsWith("http"))
            {
                ...
                if (ptn == null)
                {
                    MessageBox.Show(this, "Could not find game notation inside the link", "RTak paste error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
            }
            else
            ...
            {
                MessageBox.Show(this, "Failed to load game from notation.  " + ex.Message, "RTak paste error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        }
    }
