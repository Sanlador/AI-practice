...
namespace TakEngine
{
    public class GameState
    {
        ...
        [ThreadStatic]
        static List<IMove> PlayMoves;

        [ThreadStatic]
        static List<BoardPosition> BoardOrder;
        public static IMove Play(GameState game, string notation)
        {
            if (PlayMoves == null)
                PlayMoves = new List<IMove>();
            if (BoardOrder != null && BoardOrder.Count != game.Size * game.Size)
                BoardOrder = null;
            if (BoardOrder == null)
            {
                BoardOrder = new List<BoardPosition>();
                for (int y = 0; y < game.Size; y++)
                    for (int x = 0; x < game.Size; x++)
                        BoardOrder.Add(new BoardPosition(x, y));
            }
            Notation.MoveNotation notated;
            if (!Notation.MoveNotation.TryParse(notation, out notated))
                throw new ApplicationException("Failed to parse move notation");
            Helper.EnumerateMoves(PlayMoves, game, BoardOrder);
            foreach (var move in PlayMoves)
            {
                Notation.MoveNotation compare;
                if (!Notation.MoveNotation.TryParse(move.Notate(), out compare))
                    throw new ApplicationException("Tak engine error");
                if (notated.Compare(compare))
                {
                    move.MakeMove(game);
                    game.Ply++;
                    return move;
